...
dashboardReportStyle.textContent = '.dashboard-reports{display:grid;grid-template-columns:1.15fr .85fr;gap:14px;margin:16px 0}.dashboard-report{padding:16px 18px;border:1px solid #dce2d9;background:#fff}.dashboard-report h2{margin:0 0 3px;font-size:17px}.dashboard-report>p{margin:0 0 9px;color:#66736d;font-size:11px}.report-list{display:grid;gap:5px}.report-row{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid #e7ebe4}.report-row:first-child{border-top:0}.report-row small{display:block;margin-top:2px;color:#66736d}.stock-overview{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.stock-overview div{padding:10px;background:#f0f3ec}.stock-overview b,.stock-overview span{display:block}.stock-overview b{font-size:21px}.stock-overview span{margin-top:2px;font-size:10px;color:#66736d}@media(max-width:900px){.dashboard-reports{grid-template-columns:1fr}}';
...
(() => {
  ...
  function renderErcPreview() {
    ...
    const selectPage = document.querySelector('#ercSelectPage');
    if (selectPage) {
      const selectable = rows.filter((row) => mappedCategory(row) && row.sku && row.price);
      const selectedCount = selectable.filter((row) => ercState.selected.has(row.key)).length;
      selectPage.checked = selectable.length > 0 && selectedCount === selectable.length;
      selectPage.indeterminate = selectedCount > 0 && selectedCount < selectable.length;
      selectPage.disabled = !selectable.length;
    }
  ...
  function mountErcImport() {
    ...
    section.addEventListener('change', (event) => { if (event.target.matches('#ercImportScope')) { ercState.page = 1; renderErcPreview(); } if (event.target.matches('#ercSelectPage')) { currentPageRows().forEach((row) => { if (!mappedCategory(row) || !row.sku || !row.price) return; event.target.checked ? ercState.selected.add(row.key) : ercState.selected.delete(row.key); }); renderErcPreview(); } if (event.target.matches('[data-erc-select]')) { event.target.checked ? ercState.selected.add(event.target.dataset.ercSelect) : ercState.selected.delete(event.target.dataset.ercSelect); renderErcPreview(); } });
